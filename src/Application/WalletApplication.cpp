...
const char BYTECOIN_URI_SCHEME_NAME[] = "sikka";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("sikkawallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("sikkawallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] Sikka wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "Sikka wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have sikkad with non-default RPC port, you should terminate it and relaunch SikkaWallet\n"
      "or\n"
      "Set the Local deamon required port in SikkaWallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
