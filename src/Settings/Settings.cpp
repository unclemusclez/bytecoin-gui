...
const char DEFAULT_WALLET_FILE_NAME[] = "sikkawallet.wallet";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("sikkawallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("sikkawallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("sikkawallet.desktop");
  ...
  res = autorunSettings.contains("SikkaWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("SikkaWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.sikka.sikkawallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Sikka Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("SikkaWallet", appPath);
    } else {
      autorunSettings.remove("SikkaWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\sikka", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:sikka");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\sikka\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\sikka\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("sikkawallet.cfg"));
